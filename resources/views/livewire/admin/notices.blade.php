<div class="space-y-6">
    ...
    <div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
        <h2 class="text-lg font-semibold text-gray-900">Publish New Notice</h2>
        <p class="mt-1 text-sm text-gray-500">Send notices/notifications to libraries and students.</p>
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-4">
                <div>
                    ...
                    <select wire:model="audience"
                        class="mt-1 block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        <option value="both">Libraries + Students</option>
            ...
            @forelse ($notices as $notice)
                <div class="rounded-lg border border-gray-100 p-4">
                    <div class="flex items-center justify-between gap-3">
                        ...
                        <span class="rounded-full bg-gray-100 px-2.5 py-1 text-xs font-medium text-gray-600">{{ strtoupper($notice->audience) }}</span>
                    </div>
                    <p class="mt-2 text-sm text-gray-600">{{ $notice->body }}</p>
                    <p class="mt-2 text-xs text-gray-500">
                        By {{ $notice->creator?->name ?? 'System' }} • {{ $notice->created_at->format('M d, Y h:i A') }}
                    </p>
                </div>
            @empty
                <p class="text-sm text-gray-500">No notices published yet.</p>
            @endforelse
        </div>
